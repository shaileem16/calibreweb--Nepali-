...
def render_books_list(data, sort_param, book_id, page):
    ...
    if data == "rated":
        ...
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
                                     title=_("पुस्तकहरू"), page=website, order=order[1])
...
def render_rated_books(page, book_id, order):
    if current_user.check_visibility(constants.SIDEBAR_BEST_RATED):
        ...
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
                                     id=book_id, title=_("शीर्ष मूल्याङ्कन पुस्तकहरू"), page="rated", order=order[1])
    else:
        abort(404)
...
def render_discover_books(book_id):
    if current_user.check_visibility(constants.SIDEBAR_RANDOM):
        ...
        return render_title_template('index.html', random=false(), entries=entries, pagination=pagination, id=book_id,
                                     title=_("खोज (अनियमित पुस्तकहरू)"), page="discover")
    else:
        abort(404)
...
def render_hot_books(page, order):
    if current_user.check_visibility(constants.SIDEBAR_HOT):
        ...
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
                                     title=_("ताजा किताबहरू ((सबैभन्दा धेरै डाउनलोड गरिएको))"), page="hot", order=order[1])
    else:
        abort(404)
...
def render_downloaded_books(page, order, user_id):
    ...
    if current_user.check_visibility(constants.SIDEBAR_DOWNLOAD):
        ...
        return render_title_template('index.html',
                                     ...
                                     id=user_id,
                                     title=_(" %(user)s द्वारा डाउनलोड गरिएका पुस्तकहरू", user=user.name),
                                     page="download",
                                     order=order[1])
    else:
        abort(404)
...
def render_author_books(page, author_id, order):
    ...
    return render_title_template('author.html', entries=entries, pagination=pagination, id=author_id,
                                 title=_("लेखक: %(name)s", name=author_name), author=author_info,
...
def render_publisher_books(page, book_id, order):
    ...
    return render_title_template('index.html', random=random, entries=entries, pagination=pagination, id=book_id,
                                 title=_("प्रकाशक: %(name)s", name=publisher),
                                 page="publisher",
                                 order=order[1])
...
def render_series_books(page, book_id, order):
    ...
    return render_title_template('index.html', random=random, pagination=pagination, entries=entries, id=book_id,
                                 title=_("शृङ्खला: %(serie)s", serie=series_name), page="series", order=order[1])
...
def render_ratings_books(page, book_id, order):
    if book_id == '-1':
        ...
        title = _("मूल्याङ्कन: कुनै पनि छैन")
        rating = -1
    else:
        ...
        entries, random, pagination = calibre_db.fill_indexpage(page, 0,
                                                                ...
                                                                [order[0][0]],
                                                                True, config.config_read_column)
        title = _("मूल्याङ्कन: %(rating)s stars", rating=int(name.rating / 2))
        rating = name.rating
...
def render_formats_books(page, book_id, order):
    ...
    if name:
        entries, random, pagination = calibre_db.fill_indexpage(page, 0,
                                                                ...
                                                                True, config.config_read_column)
        return render_title_template('index.html', random=random, pagination=pagination, entries=entries, id=book_id,
                                     title=_("फाइल ढाँचा: %(format)s", format=name.format),
                                     page="formats",
                                     order=order[1])
    else:
        abort(404)
...
def render_category_books(page, book_id, order):
    if book_id == '-1':
        ...
        if tagsname:
            ...
            abort(404)
    return render_title_template('index.html', random=random, entries=entries, pagination=pagination, id=book_id,
                                 title=_("श्रेणी: %(name)s", name=tagsname), page="श्रेणी", order=order[1])
...
def render_language_books(page, name, order):
    ...
    return render_title_template('index.html', random=random, entries=entries, pagination=pagination, id=name,
                                 title=_("भाषा: %(name)s", name=lang_name), page="language", order=order[1])
...
def render_read_books(page, are_read, as_xml=False, order=None):
    ...
    if as_xml:
        ...
        if are_read:
            name = _('किताबहरू पढ्नुहोस्') + ' (' + str(pagination.total_count) + ')'
            page_name = "read"
        else:
            name = _('नपढिएका किताबहरू') + ' (' + str(pagination.total_count) + ')'
            page_name = "unread"
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
...
def render_archived_books(page, sort_param):
    ...
    name = _('संग्रहित पुस्तकहरू') + ' (' + str(len(archived_book_ids)) + ')'
    page_name = "archived"
    return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
...
def books_table():
    ...
    return render_title_template('book_table.html', title=_("पुस्तकहरूको सूची"), cc=cc, page="book_table",
...
def author_list():
    if current_user.check_visibility(constants.SIDEBAR_AUTHOR):
        ...
        return render_title_template('list.html', entries=author_copy, folder='web.books_list', charlist=char_list,
                                     title="लेखकहरू", page="authorlist", data='author', order=order_no)
    else:
        abort(404)
...
def download_list():
    ...
    if current_user.check_visibility(constants.SIDEBAR_DOWNLOAD) and current_user.role_admin():
        ...
        return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=char_list,
                                     title=_("डाउनलोडहरू"), page="downloadlist", data="download", order=order_no)
    else:
        abort(404)
...
def publisher_list():
    ...
    if current_user.check_visibility(constants.SIDEBAR_PUBLISHER):
        ...
        return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=char_list,
                                     title=_("प्रकाशकहरू"), page="publisherlist", data="publisher", order=order_no)
    else:
        abort(404)
...
def series_list():
    if current_user.check_visibility(constants.SIDEBAR_SERIES):
        ...
        if current_user.get_view_property('series', 'series_view') == 'list':
            ...
            entries = sorted(entries, key=lambda x: x[0].name.lower(), reverse=not order_no)
            return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=char_list,
                                         title=_("शृङ्खला"), page="serieslist", data="series", order=order_no)
        else:
            ...
            return render_title_template('grid.html', entries=entries, folder='web.books_list', charlist=char_list,
                                         title=_("शृङ्खला"), page="serieslist", data="series", bodyClass="grid-view",
                                         order=order_no)
    else:
        abort(404)
...
def ratings_list():
    if current_user.check_visibility(constants.SIDEBAR_RATING):
        ...
        return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=list(),
                                     title=_("मूल्याङ्कन सूची"), page="ratingslist", data="ratings", order=order_no)
    else:
        abort(404)
...
def formats_list():
    if current_user.check_visibility(constants.SIDEBAR_FORMAT):
        ...
        return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=list(),
                                     title=_("फाइल ढाँचा सूची"), page="formatslist", data="formats", order=order_no)
    else:
        abort(404)
...
def language_overview():
    if current_user.check_visibility(constants.SIDEBAR_LANGUAGE) and current_user.filter_language() == "all":
        ...
        return render_title_template('list.html', entries=languages, folder='web.books_list', charlist=char_list,
                                     title=_("भाषाहरु"), page="langlist", data="language", order=order_no)
    else:
        abort(404)
...
def category_list():
    if current_user.check_visibility(constants.SIDEBAR_CATEGORY):
        ...
        char_list = generate_char_list(entries)
        return render_title_template('list.html', entries=entries, folder='web.books_list', charlist=char_list,
                                     title=_("कोटीहरू"), page="catlist", data="category", order=order_no)
    else:
        abort(404)
